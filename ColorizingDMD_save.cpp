...
int isFrameSelected3(UINT noFr)   // return -1 if the frame is not selected, the position in the selection list if already selected, return -2 if this is the displayed one
{
...
bool isFrameSelected2(UINT noFr)   // return false if the frame is not selected
{
    for (unsigned int ti = 0; ti < nSelFrames; ti++)
    {
...
void drawrectangle(int xd, int yd, int xf, int yf, UINT8* surface, UINT8 color, BOOL isfilled, bool coloronly, byte* frame)
{
    ...
    if (y < 0) y = 0;
    if (y2 < 0) y2 = 0;
    if (x >= (int)MycRom.fWidth) x = MycRom.fWidth - 1;
    if (x2 >= (int)MycRom.fWidth) x2 = MycRom.fWidth - 1;
    if (y >= (int)MycRom.fHeight) y = MycRom.fHeight - 1;
    if (y2 >= (int)MycRom.fHeight) y2 = MycRom.fHeight - 1;
...
void Draw_Paste_Over(GLFWwindow* glfwin,UINT x,UINT y,UINT zoom)
{
    ...
    Draw_Rectangle(glfwin, x + paste_offsetx * zoom, y + paste_offsety * zoom, x + (Paste_Width + paste_offsetx) * zoom, y + (Paste_Height + paste_offsety) * zoom);
}
    ...
    for (UINT tl = 0; tl < 2; tl++)
    {
        for (UINT tk = firstfr + 1; tk < lastfr; tk++)
        {
            ...
            for (UINT ti = 1; ti < MycRom.fHeight; ti++)
            {
                ...
                if (allgood)
                {
                    CopyRows(noreffr, ti, tk, 0, MycRom.fHeight - ti);
                    break;
                }
            }
            if (allgood) continue;
            ...
            for (UINT ti = 1; ti < MycRom.fHeight; ti++)
            {
                ...
                if (allgood)
                {
                    CopyRows(noreffr, 0, tk, ti, MycRom.fHeight - ti);
                    break;
                }
            }
            if (allgood) continue;
            ...
            for (UINT ti = 1; ti < MycRom.fWidth; ti++)
            {
                ...
                if (allgood)
                {
                    CopyCols(noreffr, ti, tk, 0, MycRom.fWidth - ti);
                    break;
                }
            }
            if (allgood) continue;
            ...
            for (UINT ti = 1; ti < MycRom.fWidth; ti++)
            {
                ...
                if (allgood)
                {
                    CopyCols(noreffr, 0, tk, ti, MycRom.fWidth - ti);
                    break;
                }
            }
        }
...
bool CreateGIF(char* GIFname, unsigned char* pimages, UINT8* poimages, unsigned char* ppalettes, unsigned int* pdurations, UINT8* pcrotations, int nimages, int width, int iheight)
{
    int height = iheight;
    int offsety = 0;
    if (poimages)
    {
        height = iheight * 2 + GIF_FRAME_SEPARATION;
        offsety = iheight + GIF_FRAME_SEPARATION;
    }
    ge_GIF* gif = ge_new_gif(GIFname, width, height, 7, 64, 0);
    UINT8 lapalette[128 * 3];
    memcpy_s(&lapalette[64 * 3], 64 * 3, originalcolors, 16 * 3);
    for (int ti = 0; ti < nimages; ti++)
    {
        int nrot = 0;
        memset(gif->frame, 64, width * height);
        memcpy_s(lapalette, 128 * 3, &ppalettes[ti * 64 * 3], 64 * 3);
        ...
        if (poimages) memcpy_s(gif->frame, width * height, &poimages[ti * width * iheight], width * iheight);
        if (nrot == 0)
        {
            memcpy_s(&gif->frame[offsety * width], width * iheight, &pimages[ti * width * iheight], width * iheight);
            ge_add_frame(gif, lapalette, (pdurations[ti] + 5) / 10);
        }
        else
        {
            ...
            ge_add_frame(gif, lapalette, (lastdis + 5) / 10);
            if (oneshot) break;
            for (int tk = 0; tk < nrot; tk++)
            {
            ...
            while (1 == 1)
            {
                ...
                for (int tk = 0; tk < width * iheight; tk++)
                {
                    gif->frame[offsety * width + tk] = 255;
                    for (int tl = 0; tl < nrot; tl++)
                    {
                        if ((pimages[ti * width * iheight + tk] >= lesrot[tl * 3]) && (pimages[ti * width * iheight + tk] < lesrot[tl * 3] + lesrot[tl * 3 + 1]))
                        {
                            gif->frame[offsety * width + tk] = pimages[ti * width * iheight + tk] + acshift[tl];
                            if (gif->frame[offsety * width + tk] >= lesrot[tl * 3] + lesrot[tl * 3 + 1]) gif->frame[offsety * width + tk] -= lesrot[tl * 3 + 1];
                        }
                    }
                    if (gif->frame[offsety * width + tk] == 255) gif->frame[offsety * width + tk] = pimages[ti * width * iheight + tk];
                }
                if (nextrota >= pdurations[ti])
                {
                    ge_add_frame(gif, lapalette, max((int)(pdurations[ti] - lastdis + 5) / 10, 1));
                    break;
                }
                ge_add_frame(gif, lapalette, max((int)(nextrota - lastdis + 5) / 10, 1));
                lastdis = nextrota;
                for (int tk = 0; tk < nrot; tk++)
                {
...
void SaveAnimatedGif(void)
{
    ...
    {
        poimages = (UINT8*)malloc(nimages * MUL_SIZE_GIF * MycRom.fWidth * MUL_SIZE_GIF * MycRom.fHeight);
        if (!poimages) imok = false;
    }
    UINT32* pdurations = (UINT32*)malloc(nimages * sizeof(UINT32));
    if ((!pimages) || (!pdurations) || (!imok))
    {
    ...
    memset(pimages, 64, nimages * MUL_SIZE_GIF * MycRom.fWidth * MUL_SIZE_GIF * MycRom.fHeight);
    if (poimages) memset(poimages, 64, nimages * MUL_SIZE_GIF * MycRom.fWidth * MUL_SIZE_GIF * MycRom.fHeight);
    for (UINT ti = firstfr; ti <= lastfr; ti++)
    {
        for (UINT tj = 0; tj < MycRom.fWidth * MycRom.fHeight; tj++)
        {
            ...
            if (poimages)
            {
                pcol = MycRP.oFrames[ti * MycRom.fWidth * MycRom.fHeight + tj] + 64;
                DrawImagePix(&poimages[(ti - firstfr) * MycRom.fWidth * MUL_SIZE_GIF * MycRom.fHeight * MUL_SIZE_GIF], tj, pcol, MUL_SIZE_GIF);
            }
        }
    ...
    {
        cprintf("The GIF file was not saved");
        return;
    }
    ...
    CreateGIF(ofn.lpstrFile, pimages, poimages, &MycRom.cPal[firstfr * MycRom.ncColors * 3], pdurations, &MycRom.ColorRotations[firstfr * 3 * MAX_COLOR_ROTATION], nimages, MycRom.fWidth * MUL_SIZE_GIF, MycRom.fHeight * MUL_SIZE_GIF);
    free(pimages);
    if (poimages) free(poimages);
    free(pdurations);
}
...
LRESULT CALLBACK Tester_Proc(HWND hwDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (Msg)
    {
        ...
        {
            char tbuf[128];
            sprintf_s(tbuf, 128, "Frame Tester                   Current frame: %i / Total: %i",0,nSelFrames);
            ...
            SendFrameToTester(SelFrames[0], MycRom.noColors, MycRP.oFrames, MycRom.fWidth, MycRom.fHeight);
        ...
        case WM_TIMER:
        {
            ...
            {
                int frpos = (int)SendMessage(hChFr, TBM_GETPOS, TRUE, 0);
                if (TesterSelectionMode) SendFrameToTester(SelFrames[frpos], MycRom.noColors, MycRP.oFrames, MycRom.fWidth, MycRom.fHeight);
                else SendFrameToTester(frpos, MycRom.noColors, pTesterFrames, MycRom.fWidth, MycRom.fHeight);
            }
            return TRUE;
        }
        case WM_COMMAND:
            ...
            switch (wParam)
            {
                ...
                case IDC_LOADDUMP:
                {
                    ...
                    if (GetOpenFileNameA(&ofn) == TRUE)
                    {
                        ...
                        char tbuf[128];
                        sprintf_s(tbuf, 128, "Frame Tester                   Current frame: %i / Total: %i", 0, nTesterFrames);
                        SetWindowTextA(hwDlg, tbuf);
                        SetDlgItemTextA(hwDlg, IDC_INFO, "Navigate through the dumped frames:");
                        TesterSelectionMode = false;
                        SendFrameToTester(0, MycRom.noColors, pTesterFrames, MycRom.fWidth, MycRom.fHeight);
        ...
        case WM_HSCROLL:
        {
            ...
            if (TesterSelectionMode)
            {
                SendFrameToTester(SelFrames[frpos], MycRom.noColors, MycRP.oFrames, MycRom.fWidth, MycRom.fHeight);
                sprintf_s(tbuf, 128, "Frame Tester                   Current frame: %i / Total: %i", frpos, nSelFrames);
            }
            else
            {
                SendFrameToTester(frpos, MycRom.noColors, pTesterFrames, MycRom.fWidth, MycRom.fHeight);
                sprintf_s(tbuf, 128, "Frame Tester                   Current frame: %i / Total: %i", frpos, nTesterFrames);
            }
            SetWindowTextA(hwDlg, tbuf);
            return TRUE;
        }
...
void TestSelectedFrames(void)
{
    ...
    {
        cprintf(pidmd);
        MessageBoxA(hWnd, "Couldn't start DmdDevice64.dll from the VPinMame folder you chose, thanks to retry tester and choose another one", "Failed", MB_OK);
        Dir_VP[0] = 0;
        SavePaths();
        return;
    }
...
void AutoCopy(void)
{
    ...
    if (!pselcrc32)
    {
        cprintf("Auto copy didn't get the memory for CRC32 calculations");
        return;
    }
    for (UINT ti = 0; ti < MycRom.nFrames; ti++)
        pselcrc32[ti] = crc32_fast_mask(&MycRP.oFrames[ti * MycRom.fWidth * MycRom.fHeight], Copy_Mask, MycRom.fWidth * MycRom.fHeight);
    for (UINT ti = 0; ti < MycRom.nFrames; ti++)
    {
        if (ti == acFrame) continue;
        if (pselcrc32[acFrame] == pselcrc32[ti])
        {
            // we have found a frame with the part selected identical, we copy the colorization
            memcpy_s(&MycRom.cPal[ti * 3 * MycRom.ncColors], 3 * MycRom.ncColors, &MycRom.cPal[acFrame * 3 * MycRom.ncColors], 3 * MycRom.ncColors);
            for (UINT tj = 0; tj < MycRom.fWidth * MycRom.fHeight; tj++)
            {
                if (Copy_Mask[tj] != 0)
                {
                    MycRom.cFrames[ti * MycRom.fWidth * MycRom.fHeight + tj] = Copy_Col[tj];
                    MycRP.oFrames[ti * MycRom.fWidth * MycRom.fHeight + tj] = Copy_Colo[tj];
...
void CalcImageCropFrame()
{
    if (TxImage >= 0)
    {
        SelRatio = (float)WSelection / (float)HSelection;
...
void Draw_Image(void)
{
    ...
    if (NSelection > 0)
    {
        ...
        UINT cropminx = min(crop_ioffsetx, crop_foffsetx), cropminy = min(crop_ioffsety, crop_foffsety);
        UINT cropmaxx = max(crop_ioffsetx, crop_foffsetx), cropmaxy = max(crop_ioffsety, crop_foffsety);
        UINT cropminsx = min(crop_ioffsetx + crop_iOsizeW - 1, crop_foffsetx + crop_fOsizeW - 1), cropminsy = min(crop_ioffsety + crop_iOsizeH - 1, crop_foffsety + crop_fOsizeH - 1);
        UINT cropmaxsx = max(crop_ioffsetx + crop_iOsizeW - 1, crop_foffsetx + crop_fOsizeW - 1), cropmaxsy = max(crop_ioffsety + crop_iOsizeH - 1, crop_foffsety + crop_fOsizeH - 1);
        //UINT sizeW = cropmaxsx - cropminx + 1, sizeH = cropmaxsy - cropminy + 1;
        glVertex2i(image_posx, image_posy);
        glVertex2i(image_posx + cropminx, image_posy);
        glVertex2i(image_posx + cropminx, image_posy + image_sizeH);
        glVertex2i(image_posx, image_posy);
        glVertex2i(image_posx + cropminx, image_posy + image_sizeH);
        glVertex2i(image_posx, image_posy + image_sizeH);

        glVertex2i(image_posx + cropmaxsx, image_posy);
        glVertex2i(image_posx + image_sizeW, image_posy);
        glVertex2i(image_posx + image_sizeW, image_posy + image_sizeH);
        glVertex2i(image_posx + cropmaxsx, image_posy);
        glVertex2i(image_posx + image_sizeW, image_posy + image_sizeH);
        glVertex2i(image_posx + cropmaxsx, image_posy + image_sizeH);

        glVertex2i(image_posx + cropminx, image_posy);
        glVertex2i(image_posx + cropmaxsx, image_posy);
        glVertex2i(image_posx + cropmaxsx, image_posy + cropminy);
        glVertex2i(image_posx + cropminx, image_posy);
        glVertex2i(image_posx + cropmaxsx, image_posy + cropminy);
        glVertex2i(image_posx + cropminx, image_posy + cropminy);

        glVertex2i(image_posx + cropminx, image_posy + cropmaxsy);
        glVertex2i(image_posx + cropmaxsx, image_posy + cropmaxsy);
        glVertex2i(image_posx + cropmaxsx, image_posy + image_sizeH);
        glVertex2i(image_posx + cropminx, image_posy + cropmaxsy);
        glVertex2i(image_posx + cropmaxsx, image_posy + image_sizeH);
        glVertex2i(image_posx + cropminx, image_posy + image_sizeH);
...
void LoadWindowPosition(void)
{
    ...
    int posxi = 250, posyi = 500, widi = 500, heii = 500;
    if (ls == ERROR_SUCCESS)
    {
        DWORD size = 4;
...
bool Save_cRom(bool autosave, bool fastsave, char* forcepath)
{
    ...
    for (UINT32 ti = 0; ti < MycRom.nFrames; ti++)
    {
        ...
        else MycRom.HashCode[ti] = crc32_fast_nomask(&MycRP.oFrames[ti * MycRom.fWidth * MycRom.fHeight], &MycRom.CompMasks[MycRom.CompMaskID[ti] * MycRom.fWidth * MycRom.fHeight], MycRom.fWidth * MycRom.fHeight, MycRom.ShapeCompMode[ti]);
    }
...
void CompareAdditionalFrames(UINT nFrames, sFrames* pFrames)
{
    ...
    for (int ti = 0; ti < (int)MycRom.nFrames; ti++)
    {
        ...
        {
            MycRom.HashCode[ti] = crc32_fast_nomask(&MycRP.oFrames[MycRom.fWidth * MycRom.fHeight * ti], &MycRom.CompMasks[MycRom.CompMaskID[ti] * MycRom.fWidth * MycRom.fHeight], MycRom.fWidth * MycRom.fHeight, (BOOL)MycRom.ShapeCompMode[ti]);
    ...
    for (unsigned int ti = 0; ti < nFrames; ti++)
    {
        ...
        for (int tj = 0; tj < (int)MycRom.nFrames; tj++)
        {
            if ((MycRom.CompMaskID[tj] != 255) && filter_allmask)
            {
                ...
                {
                    achash = crc32_fast_nomask((UINT8*)pFrames[ti].ptr, &MycRom.CompMasks[MycRom.CompMaskID[tj] * MycRom.fWidth * MycRom.fHeight], MycRom.fWidth * MycRom.fHeight, (BOOL)MycRom.ShapeCompMode[tj]);
...
const char* ButtonDescription3(HWND hOver)
{
    ...
    if (hOver == GetDlgItem(hwTB3, IDC_COLORS)) return (const char*)"Choose the colors in the frame palette to copy the image colors to";
...
void ReduceRGB24ToNColorsImage(cv::Mat image, unsigned int ncolors, unsigned char* palette, unsigned char* dimage)//, unsigned char* palette, unsigned char* Image)
{
    if (image.type() != CV_8UC3)
    {
        MessageBoxA(hImages, "The image is not in the right format, can't reduce it", "Error", MB_OK);
        return;
    }
    // Cluster
    ...
    cv::Vec3b tpalette[64];
    int ncol = 0;

    // Find the n unique colors in the source image
    for (int i = 0; i < h; i++)
    {
        for (int j = 0; j < w; j++) 
        {
            cv::Vec3b color = res.at<cv::Vec3b>(i, j);
            bool found = false;
    ...
    // Fill palette
    UINT idx = 0;
    for (int ti = 0; ti < ncol;ti++)
    {
        palette[idx++] = tpalette[ti][2];
        palette[idx++] = tpalette[ti][1];
        palette[idx++] = tpalette[ti][0];
        if (idx >= ncolors * 3) break;
    }
...
void CopyImageToSelection(UINT8* palette, UINT8* image)
{
    for (UINT tk = 0; tk < nSelFrames; tk++)
    {
        for (UINT ti = 0; ti < (UINT)(image_ncolsel * 3); ti++)
        {
            MycRom.cPal[SelFrames[tk] * 3 * MycRom.ncColors + ti + image_precolsel * 3] = palette[ti];
        }
...
void CopyImageToSelectionW(UINT8* palette, UINT8* image, UINT LedW)
{
    // we look for a continuous part of the selection after the current displayed one
    ...
    for (UINT tk = 0; tk < nfr; tk++)
    {
        UINT offset;
        if (nfr == 1) offset = 0; else offset = tk * (LedW - WSelection) / (nfr - 1);
        for (UINT ti = 0; ti < (UINT)(image_ncolsel * 3); ti++)
        {
            MycRom.cPal[(preframe + tk) * 3 * MycRom.ncColors + ti + image_precolsel * 3] = palette[ti];
        }
        for (UINT tj = 0; tj < HSelection; tj++)
        {
            for (UINT ti = 0; ti < WSelection; ti++)
            {
                ...
                {
                    UINT ttk = tk;
                    if (crop_scroll_invert) ttk = nfr - 1 - tk;
                    MycRom.cFrames[(preframe + ttk) * MycRom.fWidth * MycRom.fHeight + (tj + YSelection) * MycRom.fWidth + ti + XSelection] =
                        image_precolsel + image[tj * LedW + ti + offset];
                }
            }
        }
    }
...
void CopyImageToSelectionH(UINT8* palette, UINT8* image, UINT LedH)
{
    // we look for a continuous part of the selection after the current displayed one
    unsigned int preframe = acFrame, derframe = acFrame;
    bool frfound = true;
    ...
    {
        UINT offset;
        if (nfr == 1) offset = 0; else offset = tk * (LedH - HSelection) / (nfr - 1);
        for (UINT ti = 0; ti < (UINT)(image_ncolsel * 3); ti++)
        {
            MycRom.cPal[(preframe + tk) * 3 * MycRom.ncColors + ti + image_precolsel * 3] = palette[ti];
        }
        for (UINT tj = 0; tj < HSelection; tj++)
        {
            for (UINT ti = 0; ti < WSelection; ti++)
            {
                if (Copy_Mask[(tj + YSelection) * MycRom.fWidth + ti + XSelection] > 0)
                {
                    ...
                    if (crop_scroll_invert) ttk = nfr - 1 - tk;
                    MycRom.cFrames[(preframe + ttk) * MycRom.fWidth * MycRom.fHeight + (tj + YSelection) * MycRom.fWidth + ti + XSelection] =
                        image_precolsel + image[(tj + offset) * WSelection + ti];
                }
            }
        }
    }
...
GLuint CreateTextureFromClipboard(UINT* pw, UINT* ph)
{
    ...
    *ph = bm.bmHeight;
    ...
    {
        MessageBoxA(hImages, "Unknock image format in the clipboard", "Failed", MB_OK);
        CloseClipboard();
        return (UINT) -1;
    }
...
void ScrollCopy(HWND hDlg)
{
    ...
    if (image_ncolsel < 2)
    {
        MessageBoxA(hImages, "Select at least 2 colors to copie the image palette to", "Failed", MB_OK);
        return;
    }
    UINT8* image = NULL;
    if (crop_scroll_mode == 1) image = (UINT8*)malloc(HSelection * HSelection * crop_sizeW / crop_sizeH);
    else if (crop_scroll_mode == 2) image = (UINT8*)malloc(WSelection * WSelection * crop_sizeH / crop_sizeW);
    else return;
    if (!image)
    {
    ...
    SaveAction(true, SA_DRAW);
    if (crop_scroll_mode == 1)
    {
        UINT LedW = HSelection * crop_sizeW / crop_sizeH;
        ...
        cv::Mat tmp;
        cv::resize(croppedimg, tmp, cv::Size(LedW, HSelection), 0, 0, cv::INTER_LANCZOS4);
        croppedimg.release();
        ...
        tmp.release();
        CopyImageToSelectionW(palette, image, LedW);
        EnableWindow(GetDlgItem(hDlg, IDC_SCROLLCOPY), TRUE);
    }
    else if (crop_scroll_mode == 2)
    {
        UINT LedH = WSelection * crop_sizeH / crop_sizeW;
        float ratio = (float)width_image / (float)image_sizeW;
        EnableWindow(GetDlgItem(hDlg, IDC_SCROLLCOPY), FALSE);
        cv::Rect croprect((int)(crop_offsetx * ratio), (int)(crop_offsety * ratio), (int)(crop_sizeW * ratio), (int)(crop_sizeH * ratio));
        cv::Mat croppedimg = image_mat(croprect);
        cv::Mat tmp;
        cv::resize(croppedimg, tmp, cv::Size(WSelection, LedH), 0, 0, cv::INTER_LANCZOS4);
        croppedimg.release();
        unsigned char palette[64 * 3];
        if (image_ncolsel > NSelection) image_ncolsel = NSelection;
        ReduceRGB24ToNColorsImage(tmp, image_ncolsel, palette, image);
        tmp.release();
        CopyImageToSelectionH(palette, image, LedH);
        EnableWindow(GetDlgItem(hDlg, IDC_SCROLLCOPY), TRUE);
    }
    free(image);
...
INT_PTR CALLBACK Toolbar_Proc3(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_DRAWITEM:
        {
            ...
            LPDRAWITEMSTRUCT pDIS = (LPDRAWITEMSTRUCT)lParam;
            if (pDIS->CtlID != IDC_COLORS) return FALSE; // we only owner draw the colors
            HDC hdc = GetDC(GetDlgItem(hDlg, IDC_COLORS));
            RECT rcColors, rcFirst;
            GetWindowRect(GetDlgItem(hDlg, IDC_COLORS), &rcColors);
            rcColors.right -= rcColors.left;
            rcColors.bottom -= rcColors.top;
            rcColors.left = 0;
            rcColors.top = 0;
            ...
            for (UINT tj = 0; tj < 4; tj++)
            {
                for (UINT ti = 0; ti < 16; ti++)
                {
                    ...
                    {
                        SelectObject(hdc, hPen);
                        Rectangle(hdc, rcFirst.left, rcFirst.top, rcFirst.right + 1, rcFirst.bottom + 1);
                    }
                }
            }
            DeleteObject(hPen);
        ...
        case WM_INITDIALOG:
        {
            ...
            SetWindowSubclass(GetDlgItem(hDlg, IDC_COLORS), ButtonSubclassProc3, 0, 0);
            ...
            HWND hPicControl = GetDlgItem(hDlg, IDC_COLORS);
            RECT rcPicControl;
            GetWindowRect(hPicControl, &rcPicControl);
            ...
            SendMessage(GetDlgItem(hDlg, IDC_NOSCROLL), BM_SETCHECK, BST_CHECKED, 0);
            SendMessage(GetDlgItem(hDlg, IDC_HSCROLL), BM_SETCHECK, BST_UNCHECKED, 0);
            SendMessage(GetDlgItem(hDlg, IDC_VSCROLL), BM_SETCHECK, BST_UNCHECKED, 0);
            crop_scroll_invert = false;
            SendMessageA(GetDlgItem(hDlg, IDC_HSCROLL), WM_SETTEXT, 0, (LPARAM)"R scroll");
            SendMessageA(GetDlgItem(hDlg, IDC_VSCROLL), WM_SETTEXT, 0, (LPARAM)"D scroll");
            crop_scroll_mode = 0;
        ...
        case WM_LBUTTONDOWN:
        {
            // Get the mouse position relative to the parent dialog box
            POINT ptMouse;
            ptMouse.x = GET_X_LPARAM(lParam);
            ptMouse.y = GET_Y_LPARAM(lParam);
            // Check if the mouse position is within the rectangle that defines the picture control
            RECT rcPicControl;
            GetWindowRect(GetDlgItem(hDlg, IDC_COLORS), &rcPicControl);
            MapWindowPoints(HWND_DESKTOP, hDlg, (LPPOINT)&rcPicControl, 2);
            if (PtInRect(&rcPicControl, ptMouse))
            {
                int colstepx = (rcPicControl.right - rcPicControl.left) / 16;
                int colstepy = (rcPicControl.bottom - rcPicControl.top) / 4;
                // Get the mouse position relative to the picture control
                image_precolsel = (UINT)((ptMouse.x - rcPicControl.left) / colstepx + (ptMouse.y - rcPicControl.top) / colstepy * 16);
                image_ncolsel = 1;
                InvalidateRect(GetDlgItem(hDlg, IDC_COLORS), NULL, FALSE);
            }
            return TRUE;        
        }
        ...
        case WM_LBUTTONUP:
        {
            ...
            if (PtInRect(&rcPicControl, ptMouse))
            {
                ...
                int colstepy = (rcPicControl.bottom - rcPicControl.top) / 4;
                // Get the mouse position relative to the picture control
                image_ncolsel = (UINT)((ptMouse.x - rcPicControl.left) / colstepx + (ptMouse.y - rcPicControl.top) / colstepy * 16) - image_precolsel + 1;
                InvalidateRect(GetDlgItem(hDlg, IDC_COLORS), NULL, FALSE);
            }
            return TRUE;
        }
        ...
        case WM_COMMAND:
        {
            switch (LOWORD(wParam))
            {
                ...
                case IDC_INVERT:
                {
                    ...
                    {
                        crop_scroll_invert = true;
                        SendMessageA(GetDlgItem(hDlg, IDC_HSCROLL), WM_SETTEXT, 0, (LPARAM)"L scroll");
                        SendMessageA(GetDlgItem(hDlg, IDC_VSCROLL), WM_SETTEXT, 0, (LPARAM)"U scroll");
                    }
                    else
                    {
                        crop_scroll_invert = false;
                ...
                case IDC_COPY:
                {
                    ...
                    if ((WSelection < 4) || (HSelection < 4) || (WSelection > MycRom.fWidth) || (HSelection > MycRom.fHeight))
                    {
                    ...
                    for (UINT32 tk = 0; tk < nSelFrames; tk++)
                    {
                        ...
                        for (UINT tj = 0; tj < Paste_Height; tj++)
                        {
                            for (UINT ti = 0; ti < Paste_Width; ti++)
                            {
                                ...
                                if ((condpaste) && (Paste_Colo[i + j * Paste_Width] != MycRP.oFrames[SelFrames[tk] * MycRom.fWidth * MycRom.fHeight + (tj + paste_offsety) * MycRom.fWidth + (ti + paste_offsetx)])) continue;
...
void mouse_scroll_callback3(GLFWwindow* window, double xoffset, double yoffset)
{
    ...
    if (!(GetKeyState(VK_CONTROL) & 0x8000))
    {
        ...
        if (crop_offsetx + crop_OsizeW > image_sizeW) crop_offsetx = image_sizeW - crop_OsizeW;
        else if (crop_offsetx < 0) crop_offsetx = 0;
        if (crop_offsety + crop_OsizeH > image_sizeH) crop_offsety = image_sizeH - crop_OsizeH;
        else if (crop_offsety < 0) crop_offsety = 0;
        if (crop_scroll_mode == 1)
        {
            if (crop_scroll_extend > (int)image_sizeW - (int)(crop_offsetx + crop_OsizeW)) crop_scroll_extend = (int)image_sizeW - (int)(crop_offsetx + crop_OsizeW);
        }
        else if (crop_scroll_mode == 2)
        {
            if (crop_scroll_extend > (int)image_sizeH - (int)(crop_offsety + crop_OsizeH)) crop_scroll_extend = (int)image_sizeH - (int)(crop_offsety + crop_OsizeH);
        }
    }
    else if (crop_scroll_mode > 0)
    {
        crop_scroll_extend += step;
        if (crop_scroll_extend < 0) crop_scroll_extend = 0;
        else if (crop_scroll_mode == 1)
        {
            if (crop_scroll_extend > (int)image_sizeW - (int)(crop_offsetx + crop_OsizeW)) crop_scroll_extend = (int)image_sizeW - (int)(crop_offsetx + crop_OsizeW);
        }
        else
        {
            if (crop_scroll_extend > (int)image_sizeH - (int)(crop_offsety + crop_OsizeH)) crop_scroll_extend = (int)image_sizeH - (int)(crop_offsetx + crop_OsizeH);
        }
    }
}
...
{
    if (image_mousepressed)
    {
        if ((xpos >= image_posx) && (xpos < image_posx + image_sizeW) && (ypos >= image_posy) && (ypos < image_posy + image_sizeH))
        {
            if (xpos < image_posx + crop_sizeW / 2) crop_offsetx = 0;
            else if (xpos > image_posx + image_sizeW - crop_sizeW / 2) crop_offsetx = image_sizeW - crop_sizeW;
            else crop_offsetx = (int)xpos - image_posx - crop_sizeW / 2;
            if (ypos < image_posy + crop_sizeH / 2) crop_offsety = 0;
            else if (ypos > image_posy + image_sizeH - crop_sizeH / 2) crop_offsety = image_sizeH - crop_sizeH;
...
void mouse_button_callback3(GLFWwindow* window, int button, int action, int mods)
{
    ...
    {
        image_mousepressed = true;
        mouse_move_callback3(window, xpos, ypos);
    }
    else if (action == GLFW_RELEASE) image_mousepressed = false;
}
...
bool CreateToolbar2(void)
{
    ...
    if (!hwTB2)
    {
        cprintf("Unable to create the sprite toolbar");
        return false;
    }
...
bool CreateToolbar3(void)
{
    ...
    if (!hwTB3)
    {
        cprintf("Unable to create the image toolbar");
        return false;
    }
                     ...
                     _In_ int       nCmdShow)
{
    ...
    {
        cprintf("Call to RegisterClassEx3 failed!");
        return 1;
    }
    ...
    while (!fDone)
    {
        ...
        if (!IsIconic(hWnd))
        {
            ...
            else if (GetForegroundWindow() == hImages)
            {
                ...
                //    if (isPressed(VK_LEFT, &timeLPress))
                //    {
                //       if (crop_scroll_extend > movestep) crop_scroll_extend -= movestep; else crop_scroll_extend = 0;
                //    }
                //    if (isPressed(VK_RIGHT, &timeRPress))
                //    {
                //        //if (crop_offsetx <= image_sizeW - crop_sizeW - movestep) crop_offsetx += movestep; else crop_offsetx = image_sizeW - crop_sizeW;
                //        crop_scroll_extend += movestep;
                //    }
                //}
                //else if (crop_scroll_mode == 2)
                //{
